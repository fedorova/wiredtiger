...
__wt_cache_pool_config(WT_SESSION_IMPL *session, const char **cfg)
{
	...
	if (updating)
		WT_ERR(__wt_cond_signal(
			       session,
			       __wt_process.cache_pool->cache_pool_cond,
			       false));
...
__wt_conn_cache_pool_open(WT_SESSION_IMPL *session)
{
	...
	WT_RET(__wt_cond_signal(session, cp->cache_pool_cond, false));
...
__wt_conn_cache_pool_destroy(WT_SESSION_IMPL *session)
{
	...
	if (found) {
		...
		WT_TRET(__wt_cond_signal(session, cp->cache_pool_cond, false));
