...
static int  __evict_pass(WT_SESSION_IMPL *, bool);
...
__evict_server(void *arg)
{
	...
	while (F_ISSET(conn, WT_CONN_EVICTION_RUN)) {
		...
		WT_ERR(__evict_pass(session, true));
...
__evict_worker(void *arg)
{
	...
	while (F_ISSET(conn, WT_CONN_EVICTION_RUN) &&
		...
		if ((ret = __evict_pass(session, false)) == WT_NOTFOUND) {
			WT_TRACE_RECORD(session, "worker waits");
			WT_ERR(__wt_cond_wait(
			    session, cache->evict_waiter_cond, 10000));
		}
		else {
			WT_TRACE_RECORD(session, "worker error");
			WT_ERR(ret);
		}
	}
	WT_ERR(__wt_verbose(
 ...
 */
static int
__evict_pass(WT_SESSION_IMPL *session, bool is_server)
{
	WT_CACHE *cache;
	WT_CONNECTION_IMPL *conn;
	WT_DECL_RET;
		...
		if (!locked &&
			...
			if (FLD_ISSET(cache->state, WT_EVICT_PASS_ALL |
				...
				if (conn->evict_workers >=
					...
					WT_ERR(__evict_workers_resize(session));
				worker =
			...
			WT_ERR(__wt_verbose(session, WT_VERB_EVICTSERVER,
 ...
 */
static int
__evict_clear_walk(WT_SESSION_IMPL *session, bool is_locked)
{
	WT_BTREE *btree;
	WT_CACHE *cache;
	WT_DECL_RET;
...
__evict_lru_pages(WT_SESSION_IMPL *session, bool is_server)
{
	...
	if (is_server && S2C(session)->evict_workers > 1) {
		...
		if (app_evict_percent < APP_EVICT_THRESHOLD) {
			...
			ret = 0;
			WT_TRACE_RECORD(session, "server not evicting");
			goto done;
		}
		WT_TRACE_RECORD(session, "server evicting");
	}
...
#define TRACE_STRLEN 32
	...
	if (entries == 0) {
		...
		__wt_spin_unlock(session, &cache->evict_queue_lock);
		ret = 0;
		WT_TRACE_RECORD(session, "evict_lru_walk: no entries");
		goto done;
	}
    ...
    WT_SESSION_IMPL *session, bool is_server, WT_BTREE **btreep, WT_REF **refp)
{
	...
	*refp = NULL;
	...
	if (cache->evict_current == NULL) {
		...
		WT_TRACE_RECORD(session, "get_ref empty 1");
		ret = WT_NOTFOUND;
		goto done_ret;
	}
	__wt_spin_lock(session, &cache->evict_queue_lock);
	...
	if (cache->evict_current == NULL) {
		...
		WT_TRACE_RECORD(session, "get_ref empty 2");
		ret = WT_NOTFOUND;
		goto done_ret;
	}
	/*
	...
	while ((evict = cache->evict_current) != NULL &&
		...
		++cache->evict_current;
		WT_TRACE_RECORD(session, "evict_current increment");
		...
		if (!__wt_atomic_casv32(
			...
			__evict_list_clear(session, evict);
			WT_TRACE_RECORD(session, "failed to lock page");
			continue;
		}
...
__evict_page(WT_SESSION_IMPL *session, bool is_server)
{
	...
	if (F_ISSET(session, WT_SESSION_INTERNAL)) {
		if (is_server) {
			...
			cache->server_evicts++;
			WT_TRACE_RECORD(session, "server evicts");
		} else {
			WT_STAT_FAST_CONN_INCR(
				session, cache_eviction_worker_evicting);
			cache->worker_evicts++;
			WT_TRACE_RECORD(session, "worker evicts");
		}
	} else {
		WT_STAT_FAST_CONN_INCR(session, cache_eviction_app);
		cache->app_evicts++;
		WT_TRACE_RECORD(session, "app evicts");
	}
